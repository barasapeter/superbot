...
FIXED: WebSocket close frame handling & EventStorageManager.trade_event method
"""
...
class SessionStats:
    ...
    def record(self, profit, contract_id, signal, stake, entry_price, exit_price):
        ...
        # Store trade event - FIXED: added try/except to handle any event storage issues
        if self.event_manager:
            try:
                ...
                # Silently fail - don't let event storage errors affect trading
                pass
...
class PersistentTradeManager:
    ...
    async def _safe_close_client(self, client, label="client"):
        """Safely close a client without causing 'no close frame' errors"""
        if client is None:
            return

        try:
            # Check if the client has a ws attribute and it's not already closed
            if hasattr(client, "ws") and client.ws is not None:
                try:
                    ...
                    # Ignore errors when trying to send close frame
                    pass
        ...
        if self.execution_client is None or not self.execution_client.is_connected:
            try:
                if self.execution_client:
                    await self._safe_close_client(self.execution_client, "execution")
        ...
        if self.polling_client is None or not self.polling_client.is_connected:
            try:
                if self.polling_client:
                    await self._safe_close_client(self.polling_client, "polling")
                ...
                if self.execution_client and self.execution_client.ws:
                    ...
                    if not exec_ok:
                        ...
                        await self._safe_close_client(
                            self.execution_client, "execution"
                        )
                        self.execution_client = None
    ...
    async def _resolve_trade(self, contract_id, signal, stake, entry_price, currency):
        try:
            ...
            if profit is not None:
                ...
                await self.martingale.record_result_async(
                    won=profit > 0, currency=currency
                )
            ...
            # Don't log as error for event storage issues
            if "trade_event" not in str(e) and "EventStorageManager" not in str(e):
                self.logger.error(
                    f"{C.RED}❌ Error resolving trade {contract_id}: {e}{C.RESET}"
                )
    ...
    async def close(self):
        ...
        # FIXED: Safe client closing to avoid "no close frame" errors
        if self.execution_client:
            await self._safe_close_client(self.execution_client, "execution")
            self.execution_client = None
        ...
        self.logger.info(f"{C.GREY}🔌 Connections closed.{C.RESET}")
...
async def run_worker(config, logger=None, worker_id=None):
    ...
    try:
        ...
        try:
            ...
            # FIXED: Safe close for balance client
            if balance_client and balance_client.ws is not None:
                try:
                    if not balance_client.ws.closed:
                        await balance_client.ws.close()
                except Exception:
                    pass
        ...
        try:
            ...
            # FIXED: Safe close for tick client
            if tick_client and tick_client.ws is not None:
                try:
                    if not tick_client.ws.closed:
