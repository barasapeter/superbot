...
class MartingaleManager:
    ...
    async def next_stake_async(self):
        async with self._lock:
            ...
            return round(min(self.current_stake, self.max_stake), 2)
    ...
    async def pre_fetch_next_stake(self):
        async with self._lock:
            ...
            next_stake = round(
                min(self.current_stake * self.multiplier, self.max_stake), 2
            )
...
class PersistentTradeManager:
    ...
    async def process_trade(
        self, signal, symbol, currency, martingale, stats, stop_loss_percent
    ):
        if self._is_closing:
            return
        ...
        async with self._trade_lock:
            ...
            asyncio.create_task(
                self._resolve_trade(
                    contract_id, signal, stake, entry_price, martingale, stats, currency
                )
            )

    async def _resolve_trade(
        self, contract_id, signal, stake, entry_price, martingale, stats, currency
    ):
        ...
        if profit is not None:
            ...
            self.logger.info(stats.summary_line())
            ...
            async with martingale._lock:
                martingale.current_stake = martingale.base_stake
                martingale.step = 0
    ...
    async def trade_worker(self, martingale, stats, stop_loss_percent):
        ...
        while not self._is_closing:
            try:
                ...
                await self.process_trade(
                    signal, symbol, currency, martingale, stats, stop_loss_percent
                )
            except asyncio.CancelledError:
                break
            except Exception as e:
    ...
    def queue_trade(self, signal, symbol, currency):
        if self._is_closing:
            return
        if hasattr(self, "martingale") and self.martingale._stopped:
            self.logger.warning(
...
class TickStreakTracker:
    def __init__(
        self, target_streak=4, max_allowed_latency_ms=350, cooldown_seconds=1.0
    ):
        self.prices = []
        self.timestamps = []
        self.streak = 0
    ...
    def process_new_tick(self, price, server_epoch, logger):
...
# ==================== BANNER ====================
def print_banner(config, martingale, stats):
    mg_line = (
...
{C.GREY}  Symbol:{C.RESET} {C.WHITE}{config['symbol']}{C.RESET}   {C.GREY}Base Stake:{C.RESET} {C.WHITE}{config['base_stake']} {config['currency']}{C.RESET}   {C.GREY}Target Streak:{C.RESET} {C.WHITE}{config['target_streak']}{C.RESET}   {C.GREY}Duration:{C.RESET} {C.WHITE}{config['contract_duration']} ticks{C.RESET}
  {mg_line}
  {C.GREY}Stop Loss:{C.RESET} {C.WHITE}{config['stop_loss_percent']}% drawdown{C.RESET}
  {C.GREEN}⚡ FIXED: Martingale Pre-Fetch | Dual Connections | WebSocket Recovery{C.RESET}
  {C.GREY}💓 Heartbeat: {config['heartbeat_interval']}s | Max Reconnect: {config['max_reconnect_attempts']}{C.RESET}
"""
    print(banner)
...
async def run_worker(config):
    ...
    if base_stake < min_stake:
        logger.warning(
            f"Calculated base stake from balance of {initial_balance} {currency} is {base_stake} {currency}. Resetting to absolute minimum {min_stake} {currency}... Please consider recharging your balance."
        )
        base_stake = min_stake

    full_config["base_stake"] = base_stake
    ...
    print_banner(full_config, martingale, stats)

    trade_manager = PersistentTradeManager(full_config, logger)
    ...
    worker_task = asyncio.create_task(
        trade_manager.trade_worker(martingale, stats, stop_loss_percent)
    )
    ...
    try:
        ...
        tracker = TickStreakTracker(
            ...
            cooldown_seconds=signal_cooldown,
        )
        ...
        async for message_str in tick_client.ws:
            ...
            if message.get("msg_type") == "tick":
                ...
                signal = tracker.process_new_tick(price, epoch, logger)
